...
/* timeradd 参数1，2，3全是struct timeval.行为是把参数1和参数2的时间相加，然后赋值给ev->timeout*/
...
static struct fpm_event_module_s *module;   //事件io机制的的全局变量
/* 维护两个全局队列。
 * 其中fpm_event_queue_fd队列的节点which==FPM_EV_READ
 * fpm_event_queue_timer队列的节点which==FPM_EV_TIMEOUT
 */
static struct fpm_event_queue_s *fpm_event_queue_timer = NULL;
static struct fpm_event_queue_s *fpm_event_queue_fd = NULL;   //event_queue的第一个节点
...
/* 1. 遍历检查queue,若ev已存在，则返回
 * 2. malloc struct fpm_event_queue_s,赋值后把这个节点添加到queue的头部
 * 3. 更新全局变量fpm_event_queue_t的值（该变量保存指向queue头部节点的指针）
 * 4. 往IO复用器中注册事件。如，在epoll中注册
 */
static int fpm_event_queue_add(struct fpm_event_queue_s **queue, struct fpm_event_s *ev) /* {{{ */
{
...
/* pre_init。指定该事件模块使用的io复用机制
 * kqueue is in BSD family
 * epoll is in Linux
 * /dev/poll is in Solaris
 * 
 * fpm_event_xxx_module()系列函数返回一个指向struct fpm_event_module_s的指针或者NULL
 * 
 * 返回值：成功返回0，失败返回-1
 * 根据字符串参数machanism，给全局变量module赋值
 */
...
void fpm_event_loop(int err) /* {{{ */
{
	...
	if (!err) {
        ...
        /*event loop*/
        /* 1. 取得定时器队列中最小的timeout
         * 2. 对io复用器调用wait,wait的事件是普通可读事件（即fpm_event_queue_fd队列中的事件）。wait的超时时间设为步骤1取得的timeout + 1微秒 或 1秒
         */
	while (1) {
		...
		q = fpm_event_queue_timer;
                ...
                /*此loop将fpm_event_queue_timer队列中最小的timeout赋值给ms，即最近超时的timeout
                 * TODO: 将 fpm_event_queue_timer 改为升序链表。可避免在读取时的遍历
                 */
		while (q) {
			if (!timerisset(&ms)) {
                            /* 若ms为空，将队列中第一个事件的timeout赋值给ms */
				ms = q->ev->timeout;
			} else {
...
/* 如果ev是数据触发的事件，则直接调用fpm_event_queue_add()注册事件
 * 如果ev是定时事件，则设置ev->frequency并获取当前时间，然后设置定时器，最后调用fpm_event_queue_add()注册事件
 */
int fpm_event_add(struct fpm_event_s *ev, unsigned long int frequency) /* {{{ */
{
	...
	ev->which = FPM_EV_TIMEOUT;
        ...
        /*fpm_clock_get() 是对系统函数gettimeofday()的包装。now被赋值为当前时间 */
	fpm_clock_get(&now);
	if (frequency >= 1000) {
