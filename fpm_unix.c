...
int fpm_unix_init_main() /* {{{ */
{
	struct fpm_worker_pool_s *wp;
        /*获得进程的euid。 uid表示该进程由哪个用户在运行，euid表示该进程拥有哪个用户的访问权限。通常euid和uid是相同的，但可以改变
         *root用户的euid为0
         */
	int is_root = !geteuid();

        /* 定义了该进程打开文件描述符的最大数量，这个资源也定义了这个进程最多能跟多少个unix domain sockets IPC
         * 其中setrlimit函数执行了设定过程
         */
	if (fpm_global_config.rlimit_files) {
        ...
        /* 定义了该进程core dump file的最大size
         * core dump file生成于： 类型为Core的信号的默认行为——中止程序然后生成一个core dump file。这类信号包括：SIGQUIT,SIGILL,SIGABRT,SIGFPE,SIGSEGV
         * core dump file的内容是 进程中止的时刻，该进程内存的映象(image)
         * core dump file被debugger（如gdb）作为分析工具
         */
