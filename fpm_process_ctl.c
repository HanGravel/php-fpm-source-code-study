...
static void fpm_pctl_check_request_timeout(struct timeval *now) /* {{{ */
{
	...
	for (wp = fpm_worker_all_pools; wp; wp = wp->next) {
		int terminate_timeout = wp->config->request_terminate_timeout;  //单个请求的超时中止时间
		int slowlog_timeout = wp->config->request_slowlog_timeout;      //当一个请求超过该设置时间后，就将对应的PHP调用堆栈信息完整写入到慢日志中
		struct fpm_child_s *child;
...
void fpm_pctl_heartbeat(struct fpm_event_s *ev, short which, void *arg) /* {{{ */
{
	...
	if (which == FPM_EV_TIMEOUT) {
		fpm_clock_get(&now);
                /* 检查每个子进程是否超时。
                 * 若超过config的request_terminate_timeout则中止该子进程。
                 * 若超过config的request_slowlog_timeout则将该子进程的堆栈信息写入到日志。*/
